namespace Mock.Tools.Controls
{
    ...
    internal class WindowObject : WinObject
    {
        ...
        internal void CloseTo()
        {
            closeTo = true;
            int pId = -1;
            try
            {
                Process p = Process.GetProcessesByName(Config.SoftwareProcessName)[0];
                pId = p.Id;
            }
            catch { }

            TaistTaskCollection task = new TaistTaskCollection();
            task.Add(() =>
                {
                    while (true)
                    {
                            ...
                            while (true)
                            {
                                try
                                {
                                    ...
                                    foreach (WindowsUnit window in activeWindowList)
                                    {
                                        try
                                        {
                                            if (!element.Equals(window) && !window.Current.AutomationId.Contains("NavForm") && window.Current.ProcessId == pId)
                                            {
                                                WindowPattern wp = window.GetCurrentPattern(WindowPattern.Pattern) as WindowPattern;
                                                wp.SetWindowVisualState(WindowVisualState.Normal);
                                                NativeMethods.PostMessage(window.Current.NativeWindowHandle, WindowsMessages.WM_SYSCOMMAND, (int)WindowsMessages.SC_CLOSE, 0);
                                                wp.Close();
                                            }
                                        }
                                        catch { }
                                ...
                                {
                                    LogManager.Debug("Warning window exist");
                                }
                                catch (WindowIsInactiveException)
                                {
                                    #region old soluation
                                    List<WindowsUnit> activeWindowList = GetActiveWindow();
                                    foreach (WindowsUnit window in activeWindowList)
                                    {
                                        try
                                        {
                                            ...
                                            if (string.Equals(window.Current.AutomationId, element.Current.AutomationId))
                                            {
                                                continue;
                                            }
                                            else
                                            {
                                                if (string.Equals("WindowsForms10.Window.8.app.0.141b42a_r12_ad1", window.Current.ClassName))
                                                {
                                                    AutomationElementCollection collection = window.FindAll(TreeScope.Children, new PropertyCondition(AutomationElement.ControlTypeProperty, ControlType.Button));
                                                    if (collection.Count == 3)
                                                    {
                                                        InvokePattern pattern = collection[2].GetCurrentPattern(InvokePattern.Pattern) as InvokePattern;
                                                        pattern.Invoke();
                                                    }
                                                    throw new ProgramAbortException();
                                                }
                                                else if (window.Current.AutomationId == "SysMessageBox")
                                                {

                                                }
                                                else if (window.Current.AutomationId == "CusMessageBox")
                                                {

                                                }
                                                else if (window.Current.ClassName == "#32770")
                                                {
                                                    MessageBoxObject msgBoxObj = new MessageBoxObject(window);
                                                    msgBoxObj.Click(true);
                                                }
                                                else
                                                {
                                                    WindowPattern wp = window.GetCurrentPattern(WindowPattern.Pattern) as WindowPattern;
                                                    ...
                                                    wp.Close();
                                                    ManualUpdateActiveWindow();
                                                    WindowsUnit titleBar = window.FindFirst(TreeScope.Children, new PropertyCondition(AutomationElement.AutomationIdProperty, "TitleBar"));
                                                    WindowsUnit closeBtn = titleBar.FindFirst(TreeScope.Children, new PropertyCondition(AutomationElement.AutomationIdProperty, "Close"));
                                                    if (closeBtn == null)
                                                    {
                                                        throw new NullControlException(window.Current.Name, "Close");
                                                    }

                                                    InvokePattern invPattern = closeBtn.GetCurrentPattern(InvokePattern.Pattern) as InvokePattern;
                                                    if (invPattern == null)
                                                    {
                                                        throw new UnSupportPatternException();
                                                    }
            ...
            try
            {
                task.Run(TaistTaskType.COMPETE);
            }
            finally
            {
                closeTo = false;
            ...
            //    {
            //        closeTo = false;
            //        Robot.StopJobs("closeToJob");
            //        throw new TimeOutException("CloseTo");
            //    }
            ...
            //                    continue;
            //                }
            //                if (window.Current.AutomationId == "SysMessageBox")
            //                {
            //                    exWindow = true;
            //                    HxDialogObject hxDlgObj = new HxDialogObject(window);
            //                    hxDlgObj.Click(true);
            //                }
            //                else if (window.Current.AutomationId == "CusMessageBox")
            //                {
            //                    exWindow = true;
            //                    HxMessageBoxObject hxMsgObj = new HxMessageBoxObject(window);
            //                    hxMsgObj.Close();
            //                }
            //                else if (window.Current.ClassName == "#32770")
            //                {
            ...
            //        //Robot.Recess(500);
            //        Search();
            //        LogManager.Debug("Close Success");
            //        closeTo = false;
            //        Robot.StopJobs("closeToJob");
            //        break;
            //    }
            //    catch (Exception ex)
            //    {
            //        #region WarningWindowExistException
            //        if (ex is WarningWindowExistException)
            //        {
            //            List<WindowsUnit> activeWindowList = GetActiveWindow();
            //            foreach (WindowsUnit window in activeWindowList)
            //            {
            ...
            //        #region WindowIsInactiveException
            //        else if (ex is WindowIsInactiveException)
            //        {
            //            List<WindowsUnit> activeWindowList = GetActiveWindow();
            //            foreach (WindowsUnit window in activeWindowList)
            //            {
            ...
            //                            msgBoxObj.Click(true);
            //                        }
            //                        else if (string.Equals("WindowsForms10.Window.8.app.0.141b42a_r12_ad1", window.Current.ClassName))
            //                        {
            //                            AutomationElementCollection collection = window.FindAll(TreeScope.Children, new PropertyCondition(AutomationElement.ControlTypeProperty, ControlType.Button));
            //                            if(collection.Count == 3)
            //                            {
            ...
            //                                if (closeBtn == null)
            //                                {
            //                                    throw new NullControlException(tmp, "Close");
            //                                }
        ...
        internal void Click(string elementName)
        {
            ...
            foreach (AutomationElement child in childCollection)
            {
                ...
                //try
                //{
                //    InvokePattern pattern = child.GetCurrentPattern(InvokePattern.Pattern) as InvokePattern;
                //    if (pattern == null)
                //    {
                //        SetCursor(child, true);
                //    }
